...
import { RotateCw, ArrowLeft, Upload, X, Download, Loader, FileText, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { PDFDocument, degrees } from "pdf-lib"; // Fixed import
...
export default function RotatePdf() {
    ...
    const handleRotate = async () => {
        ...
        try {
            ...
            downloadPdf(pdfBytes, `rotated-${file.name}`);

        } catch (e) {
            console.error(e);
            toast({ title: "Failed", description: "Could not rotate PDF.", variant: "destructive" });
        } finally {
            ...
            <main className="container-custom py-12 sm:py-16">
                <div className="mx-auto max-w-2xl">
                    <div className="mb-8 text-center sm:text-left">
                        <h1 className="text-4xl font-bold sm:text-5xl bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent inline-block">Rotate PDF</h1>
                        <p className="mt-2 text-lg text-muted-foreground">Rotate your PDF pages permanently.</p>
                    </div>
                    ...
                    {!file ? (
                        ...
                        >
                            ...
                            <h3 className="mt-4 text-xl font-semibold">Upload PDF to Rotate</h3>
                            ...
                            <div className="space-y-6">
                                <div className="flex justify-center gap-4">
                                    ...
                                    <Button
                                        ...
                                        className="flex-col h-auto py-4 px-6 gap-2"
                                    >
                                        <RotateCw className="h-6 w-6 -scale-x-100" />
                                        Left 90°
                                    </Button>
                                    <Button
                                ...
                                <Button onClick={handleRotate} disabled={isProcessing} className="w-full" size="lg">
                                    {isProcessing ? <><Loader className="mr-2 h-4 w-4 animate-spin" /> Processing...</> : <><Download className="mr-2 h-4 w-4" /> Download Rotated PDF</>}
                                </Button>
                            </div>
                        </div>
                    )}
